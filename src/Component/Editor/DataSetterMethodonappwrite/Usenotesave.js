    ...
    const handleSave = useCallback(async (currentEditor) => {
        ...
        try {
            ...
            if (currentNoteId) {
                ...
                if (updatedNote) {
                    // Override server time with local time to fix server clock drift issues (e.g. Docker sleep drift)
                    const localTime = new Date().toISOString();
                    const localUpdatedNote = {
                        ...updatedNote,
                        $updatedAt: localTime
                    };
                    dispatch(updateNoteInSlice(localUpdatedNote));
                }
                ...
                if (response?.$id) {
                    ...
                    dispatch(setnoteid(localResponse.$id));
                    dispatch(addNoteToTop(localResponse));
                }
            }
